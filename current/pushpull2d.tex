...
In this section we show NP-hardness for Push-$k$ Pull-$l$ in 3D for all positive integers $k, l$ in Section~\ref{3DNPhard} and Push-$q$ Pull-$r$ in 2D with thin walls for all positive integers $q$, $r$ in Section~\ref{2DNPhard}. We reduce from 3SAT. To do so, we introduce a new gadget called a Set-Verify and show how to construct variable, clause, and crossover gadgets out of them. We then give constructions of the gadget out of Push-Pull block puzzles.
