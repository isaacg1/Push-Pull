...
\begin{proof}
    ...
    Now, when the robot attempts to traverse the clause gadgets to reach the goal cell, the situation is simpler. A clause is traversable if and only if at least one of its literals has been set to Set. No interaction of the verification lines ($V_i - V_o$) will affect an Unset literal, and so the series of clause gadgets can only be traversed if every clause has a satisfied literal. Therefore, the robot can reach the goals if and only if the setting of variables demonstrated by the traversal of the variable gadgets satisfies the 3SAT formula encoded by the clauses.
