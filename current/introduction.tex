...
Push-$k$ Pull-$j$ is defined as follows: There is a square lattice of cells. Each cell is connected to its orthogonal neighbors. Cells may either be empty, hold a movable block, or hold a fixed block. Additionally, in settings that allow thin walls, there may be thin wall on the edge between two cells. There is also a robot in the maze. The robot may move from cell to cell, if it is not impeded by blocks or walls. The robot may also push up to $k$ movable blocks arranged in a straight line one cellforward, as long as there is an open cell with no wall in that direction. Likewise, the robot may pull up to $j$ movable blocks as long as there are no walls in the way and there is an open cell behind the robot. The goal of the puzzle is for the robot to reach a specified goal cell. We will analyze the problem of deciding whether a given puzzle is solvable or not.
